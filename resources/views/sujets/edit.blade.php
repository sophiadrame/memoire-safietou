...
<div style="max-width:700px;">
    ...
    <div style="background:#fff; border-radius:16px; border:1px solid #f1f5f9; box-shadow:0 1px 3px rgba(0,0,0,.04); padding:28px;">
        <h2 style="font-family:'Syne',sans-serif; font-weight:700; color:#1e293b; font-size:17px; margin:0 0 20px; padding-bottom:16px; border-bottom:1px solid #f1f5f9;">
            <i class="fa-solid fa-pen" style="color:#1e40af; margin-right:8px;"></i>Modifier le sujet
        </h2>

        <form action="{{ route('sujets.update', $sujet) }}" method="POST" style="display:flex; flex-direction:column; gap:18px;">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="titre" value="{{ old('titre', $sujet->titre) }}" required
            ...
            <div>
                ...
                <textarea name="description" rows="5" required
                          style="width:100%; padding:10px 14px; border:1px solid #e2e8f0; border-radius:10px; font-size:14px; outline:none; box-sizing:border-box; resize:none;">{{ old('description', $sujet->description) }}</textarea>
            </div>
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:16px;">
                <div>
                    ...
                    <select name="type" required style="width:100%; padding:10px 14px; border:1px solid #e2e8f0; border-radius:10px; font-size:14px; outline:none; box-sizing:border-box; color:#475569;">
                        ...
                        <option value="{{ $t }}" {{ old('type', $sujet->type) === $t ? 'selected' : '' }}>{{ $t }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="filiere" required style="width:100%; padding:10px 14px; border:1px solid #e2e8f0; border-radius:10px; font-size:14px; outline:none; box-sizing:border-box; color:#475569;">
                        ...
                        <option value="{{ $f }}" {{ old('filiere', $sujet->filiere) === $f ? 'selected' : '' }}>{{ $f }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="niveau" required style="width:100%; padding:10px 14px; border:1px solid #e2e8f0; border-radius:10px; font-size:14px; outline:none; box-sizing:border-box; color:#475569;">
                        ...
                        <option value="{{ $n }}" {{ old('niveau', $sujet->niveau) === $n ? 'selected' : '' }}>{{ $n }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                <select name="statut" style="width:100%; padding:10px 14px; border:1px solid #e2e8f0; border-radius:10px; font-size:14px; outline:none; box-sizing:border-box; color:#475569;">
                    @foreach(['disponible','pris','archivé'] as $s)
                    <option value="{{ $s }}" {{ old('statut', $sujet->statut) === $s ? 'selected' : '' }}>{{ ucfirst($s) }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <select name="encadreur_id" required style="width:100%; padding:10px 14px; border:1px solid #e2e8f0; border-radius:10px; font-size:14px; outline:none; box-sizing:border-box; color:#475569;">
                    ...
                    <option value="{{ $enc->id }}" {{ old('encadreur_id', $sujet->encadreur_id) == $enc->id ? 'selected' : '' }}>
                        {{ $enc->name }}
            ...
            <div>
                ...
                <input type="text" name="mots_cles" value="{{ old('mots_cles', $sujet->mots_cles) }}"
